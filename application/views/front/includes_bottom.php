    ...
    function set_loggers(){
        ...
        state.success(function (data) {
            if(data == 'hypass'){
				...
				?>
                +'    <li>'
                +'        <a data-toggle="modal" data-target="#v_registration" class="point">Tienda</a>'
                +'    </li>'
				<?php
					}
				?>
                +'    <li>'
                +'        <a data-toggle="modal" data-target="#login" class="point">Clientes</a>'
                +'    </li>'
                +'    <li>'
                +'       <a data-toggle="modal" data-target="#registration" class="point">Registrarse</a>'
                +'    </li>'
                +'';
            }
        });  
    ...
    $( document ).ready(function() {
		...
		?>
			<?php if($this->session->flashdata('alert') == 'successful_signup'){ ?>
				...
				setTimeout(function(){ notify('Te has registrado correctamente','success','bottom','right');}, 800);
			<?php } ?>
			<?php if($this->session->flashdata('alert') == 'successful_signin'){ ?>
				setTimeout(function(){ sound('successful_login');}, 800);  
                setTimeout(function(){ notify('Has iniciado sesión correctamente','success','bottom','right');}, 800);
			<?php } ?>
            <?php if($this->session->flashdata('alert') == 'successful_signout'){ ?>
                setTimeout(function(){ sound('successful_logout');}, 800);  
                setTimeout(function(){ notify('Has salido correctamente','success','bottom','right');}, 800);
            <?php } ?>
            <?php if($this->session->flashdata('alert') == 'unsuccessful_stripe'){ ?>
                setTimeout(function(){ sound('successful_login');}, 800);  
                setTimeout(function(){ notify('Ha ocurrido un error, por favor intentelo nuevamente','success','bottom','right');}, 800);
            <?php } ?>
		<?php } ?>
    });
