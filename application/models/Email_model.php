...
class Email_model extends CI_Model
{
     ...
     /*	
	 *	Desarrollado por: Oscar Leguizamón
	 *	Date	: 2016
	 *	Encuentralo pronto
	 *	http://marketerosweb.com
	 */
    ...
    function password_reset_email($account_type = '', $id = '', $pass = '')
    {
        ...
        if ($query->num_rows() > 0) {
            $email_msg = "Tu tippo de cuenta es : " . $account_type . "<br />";
            $email_msg .= "Tu contraseña es : " . $pass . "<br />";
            $email_sub = "Reestablecer contraseña";
            $from      = $system_email;
    ...
    function status_email($account_type = '', $id = '')
    {
        ...
        if ($query->num_rows() > 0) {
            $email_msg = "Tu tipo de cuenta es : " . $account_type . "<br />";
            if($query->row()->status == 'approved'){
                $email_msg .= "Tu cuenta ha sido : Aprovada<br />";
            } else {
                $email_msg .= "tu cuenta ha sido : Pospuesta<br />";
            }
            $email_sub = "Cambio estado de cuenta";
            $from      = $system_email;
            $from_name = $system_name;
            $email_to  = $query->row()->email;
            $this->do_email($email_msg, $email_sub, $email_to, $from);
    ...
    function membership_upgrade_email($vendor)
    {
        ...
        if ($query->num_rows() > 0) {
            if($query->row()->membership == '0'){
                $email_msg = "Tu tipo de cuenta es : Gratuita <br />";
            } else {
                $email_msg = "Tu tipo de cuenta ha sido actualizada a : " . $this->db->get_where('membership',array('membership_id'=>$query->row()->membership))->row()->title . "<br />";
            }
            $email_sub = "Actualización del tipo de cuenta";
            $from      = $system_email;
            $from_name = $system_name;
            $email_to  = $query->row()->email;
            $this->do_email($email_msg, $email_sub, $email_to, $from);
    ...
    function account_opening($account_type = '', $email = '', $pass = '')
    {
        ...
        if ($query->num_rows() > 0) {
            $password  = $pass;
            $email_msg = "Gracias por registrarte en Encuentralopronto <br />";
            $email_msg = "Tu tipo de cuenta es : " . $account_type . "<br />";
            $email_msg .= "Tu contraseña es : " . $pass . "<br />";
            if($account_type == 'vendor'){
                $email_msg .= "Ingresa en el siguiente link: <a href='".base_url()."index.php/vendor/'>".base_url()."index.php/vendor</a>";
                $email_msg .= "<br>Tu cuenta esta en .";
            }
            if($account_type == 'user'){

            }
            if($account_type == 'admin'){
                $email_msg .= "Ingresa en el siguiente link : <a href='".base_url()."index.php/admin/'>".base_url()."index.php/admin</a>";
            }
            $email_sub = "Apertura de cuenta";
            if ($account_type == 'admin') {
