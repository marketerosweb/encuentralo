...
class Home extends CI_Controller
{
	...
	/*	
	 *	Desarrollado por: Ing Oscar Leguizamon / Marketerosweb
	 *	Date	: 08 Agosto, 2016
	 *	Encuentralopronto
	 *	http://marketerosweb.co
	 */
                        ...
                        $this->router->fetch_method() == 'blog' || 
                            $this->router->fetch_method() == 'blog_view' || 
                                $this->router->fetch_method() == 'all_vendor' || 
                                    $this->router->fetch_method() == 'professional_services' || 
										$this->router->fetch_method() == 'professional_item' || 
    ...
    function all_vendor(){
        ...
        $page_data['page_name']        = "vendor/all_store";
        $page_data['asset_page']       = "all_vendor";
        $page_data['page_title']       = "Todas las tiendas";
        $this->load->view('front/index', $page_data);
    }
    function professional_services(){
        ...
        $page_data['page_name']        = "professional/professional_services";
        $page_data['asset_page']       = "professional_services";
        $page_data['page_title']       = "Servicios profesionales";
        $this->load->view('front/index', $page_data);
    }
    ...
    function product_view($para1 = "")
    {
		...
		/* FUNCTION: Loads professional View Page */
    function professional_item($para1 = "")
    {
        $page_data['page_name']    = "professional_item";
		
        $professional_data       = $this->db->get_where('professional_service', array(
            'id_proffesional' => $para1,
            'status' => 'approved'
        ));
		
        $this->db->where('id_proffesional', $para1);
		$this->db->update('professional_service', array(
            'view_profile' => 'view_profile'+1
        ));
        $page_data['professional_data'] = $professional_data->result_array();
		$page_data['page_title']   = $professional_data->row()->nombre;
